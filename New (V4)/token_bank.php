...
if (isset($_POST['bank_tokens'])) {
	if($user->account_lock){
		$session->message("<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'>×</button>Sorry, you can't do that while your account lock is enabled.</div>");
		...
		<div class="col-md-3">
			<form action="<?php echo $location; ?>" method="post">
	         <h3>Deposit Tokens</h3>
				<input type="text" name="bank_tokens" class="center" required="required" value="<?php echo htmlspecialchars($bank_tokens); ?>" />
		...
		<div class="col-md-3">
			<form action="<?php echo $location; ?>" method="post">
	         <h3>Withdraw Tokens</h3>
				<input type="text" id="withdraw_tokens" name="withdraw_tokens" class="center" required="required" value="<?php echo htmlspecialchars($withdraw_tokens); ?>" />
