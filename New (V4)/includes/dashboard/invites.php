...
if (isset($_POST['create_invite'])) {
	if($user->account_lock == 0){
		...
		$session->message("<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'>×</button>Sorry, you can't create an invitation while your account lock is active. Please disable your account lock and try again.</div>");
...
if((!empty($_GET['delete_code']))){
	if($user->account_lock == 0){
		...
		$session->message("<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'>×</button>Sorry, you can't delete an invitation while your account lock is active. Please disable your account lock and try again.</div>");
		redirect_to($location);
	}
}
