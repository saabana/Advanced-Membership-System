	...
	if($page == "login"){
		...
		if(isset($_POST['name'])){
			...
			if(in_array($name, $allowed)){
				...
				if($user->account_lock == 0){
					if($name == "username" || $name == "email"){
						...
						$user_id = $_SESSION['masdyn']['ams']['user_id'];
						// $data = User::find_by_sql("SELECT ip_whitelist FROM users WHERE user_id = '{$user_id}' LIMIT 1 ");
						// if(empty($data[0]->ip_whitelist)){
						if(empty($user->ip_whitelist)){
							$session->message("<div class='alert alert-error'><button type='button' class='close' data-dismiss='alert'>×</button>Sorry, we can't enable IP Protection on this account while your whitelist is empty.</div>");
							echo "failure";
							$flag = false;
						} else {
							$flag = true;
					...
					$session->message("<div class='alert alert-error'><button type='button' class='close' data-dismiss='alert'>×</button>Sorry, we are unable to change your settings while your account lock is active. Please deactivate your account lock and try again.</div>");
					echo "failure";
				}
			}
			...
			if($select == "ip_protection"){
				...
				echo '<div class="styled-select"><select name="ip_protection" id="ip_protection" class="span12 chzn-select" required="required" value="<?php echo $ip_protection ?>"><option value="Disabled" ';
