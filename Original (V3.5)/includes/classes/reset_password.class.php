	...
	public static function check_confirm_link($email, $hash) {
		...
		if ($rows) {
			...
			if ($rows) {
				...
				$session = new Session();
				$session->message("<div class='alert alert-error'><button type='button' class='close' data-dismiss='alert'>×</button>Sorry, but no password link was found.</div>");
				redirect_to('login.php');
			}
		} else {
			// Throw error back to the user.
			$session = new Session();
			$session->message("<div class='alert alert-error'><button type='button' class='close' data-dismiss='alert'>×</button>Sorry, but that account was not found.</div>");
			redirect_to('login.php');
		}
	}
