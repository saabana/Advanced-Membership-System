...
if(isset($_POST['delete_logs'])){
	if($user->account_lock == 0){
		...
		$session->message("<div class='alert alert-error'><button type='button' class='close' data-dismiss='alert'>×</button>Sorry, but no access logs can be found for this account.</div>");
		}
		redirect_to($location);
	} else {
		$session->message("<div class='alert alert-error'><button type='button' class='close' data-dismiss='alert'>×</button>Sorry, you can't delete your access logs while your account lock is active. Please disable your account lock and try again.</div>");
		redirect_to($location);
	}
}
